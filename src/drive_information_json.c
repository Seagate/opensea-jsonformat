...
void create_Time_String(char *timeString, uint8_t years, uint16_t days, uint8_t hours, uint8_t minutes, uint8_t seconds)
{
    ...
    if (years > 0)
    {
        snprintf(temp, sizeof(temp), "%u year%s", years, (years > 1) ? "s" : "");
    ...
    if (days > 0)
    {
        if (strlen(timeString) > 0)
            strcat(timeString, ", ");
        snprintf(temp, sizeof(temp), "%u day%s", days, (days > 1) ? "s" : "");
        strcat(timeString, temp);
    }
    if (hours > 0)
    {
        if (strlen(timeString) > 0)
            strcat(timeString, ", ");
        snprintf(temp, sizeof(temp), "%u hour%s", hours, (hours > 1) ? "s" : "");
        strcat(timeString, temp);
    }
    if (minutes > 0)
    {
        if (strlen(timeString) > 0)
            strcat(timeString, ", ");
        snprintf(temp, sizeof(temp), "%u minute%s", minutes, (minutes > 1) ? "s" : "");
        strcat(timeString, temp);
    }
    if (seconds > 0)
    {
        if (strlen(timeString) > 0)
            strcat(timeString, ", ");
        snprintf(temp, sizeof(temp), "%u second%s", seconds, (seconds > 1) ? "s" : "");
        strcat(timeString, temp);
    }
}
                                                                ...
                                                                ptrDriveInformation driveInfo)
{
    ...
    {
        create_JSON_For_Device_Information(nodeInfoObject, translatorDriveInfo);
        json_object_object_add(rootObject, "SCSI Translator Reported Information", nodeInfoObject);
    }
    else
    {
        json_object_object_add(rootObject, "SCSI Translator Information", json_obejct_new_string("Not Available"));
    }
    if (driveInfo != M_NULLPTR && driveInfo->infoType == DRIVE_INFO_SAS_SATA)
    {
        create_JSON_For_Device_Information(nodeInfoObject, driveInfo);
    ...
    {
        json_object_object_add(rootObject, "Drive Information", json_obejct_new_string("Not Available"));
    }
...
eReturnValues create_JSON_Node_For_SAS_Sata_Device_Information(json_object* rootObject, ptrDriveInformationSAS_SATA driveInfo)
{
    ...
    if (driveInfo->worldWideNameSupported)
    {
        ...
        snprintf(worldWideName, sizeof(worldWideName), "%016" PRIX64, driveInfo->worldWideName);

        if (driveInfo->worldWideNameExtensionValid)
        {
            snprintf(worldWideName, sizeof(worldWideName), "%016" PRIX64 "%016" PRIX64, driveInfo->worldWideName,
    ...
    if (driveInfo->dateOfManufactureValid) {
        ...
        snprintf(dateOfManufacture, sizeof(dateOfManufacture), "Week %" PRIu8 ", %" PRIu16, driveInfo->manufactureWeek, driveInfo->manufactureYear);
    ...
    snprintf(driveCapacity, sizeof(driveCapacity), "%0.02f/%0.02f", mCapacity, capacity);
    ...
    if (!(driveInfo->nativeMaxLBA == 0 || driveInfo->nativeMaxLBA == UINT64_MAX))
    {
        ...
        snprintf(nativedriveCapacity, sizeof(nativedriveCapacity), "%0.02f/%0.02f", mCapacity, capacity);
    ...
    if (driveInfo->powerOnMinutesValid)
    {
        ...
        char   poh[64];
        snprintf(poh, sizeof(poh), "%0.02f", powerOnHours);
    ...
    if (driveInfo->ataLegacyCHSInfo.legacyCHSValid && driveInfo->maxLBA == 0)
    {
        // Default CHS
        char defaultCHS[20];
        snprintf(defaultCHS, sizeof(defaultCHS), "%" PRIu16 " | %" PRIu8 " | %" PRIu8,
        ...
        char currentCHS[20];
        snprintf(currentCHS, sizeof(currentCHS), "%" PRIu16 " | %" PRIu8 " | %" PRIu8,
    ...
    if (driveInfo->dstInfo.informationValid && driveInfo->powerOnMinutesValid)
    {
        if (driveInfo->powerOnMinutes - (driveInfo->dstInfo.powerOnHours * 60) != driveInfo->powerOnMinutes)
        {
            ...
            {
                char timeLastDst[10];
                snprintf(timeLastDst, sizeof(timeLastDst), "%0.02f", timeSinceLastDST);
            ...
            snprintf(statusResult, sizeof(statusResult), "0x%" PRIX8, driveInfo->dstInfo.resultOrStatus);
            json_object_object_add(dstDataObj, "DST Status/Result", json_object_new_string(statusResult));

            char testRun[10];
            snprintf(testRun, sizeof(testRun), "0x%" PRIX8, driveInfo->dstInfo.testNumber);
    ...
    if (driveInfo->interfaceSpeedInfo.speedIsValid)
    {
        ...
        if (driveInfo->interfaceSpeedInfo.speedType == INTERFACE_SPEED_SERIAL)
        {
            if (driveInfo->interfaceSpeedInfo.serialSpeed.numberOfPorts > 0)
            {
                ...
                else
                {
                    ...
                    {
                        char portString[25];
                        snprintf(portString, sizeof(portString), "%" PRIu8, portIter);
                        ...
                        {
                            snprintf(portString, sizeof(portString), "%" PRIu8 " (Current Port)", portIter);
                        }
        ...
        {
            char temp[100];
            snprintf(temp, sizeof(temp), "%0.02f", driveInfo->interfaceSpeedInfo.parallelSpeed.maxSpeed);
            if (driveInfo->interfaceSpeedInfo.parallelSpeed.maxModeNameValid)
            {
                snprintf(temp, sizeof(temp), "%0.02f (%s)", driveInfo->interfaceSpeedInfo.parallelSpeed.maxSpeed,
            ...
            {
                snprintf(temp, sizeof(temp), "%0.02f", driveInfo->interfaceSpeedInfo.parallelSpeed.negotiatedSpeed);
                if (driveInfo->interfaceSpeedInfo.parallelSpeed.negModeNameValid)
                {
                    snprintf(temp, sizeof(temp), "%0.02f (%s)",
...
#    define MINUTES_IN_1_YEAR 525600.0
            ...
            char usage[64];
            snprintf(usage, sizeof(usage), "%0.02f", calculatedUsage);
    ...
    if (driveInfo->totalBytesRead > 0)
    {
        ...
        char bytesRead[64];
        snprintf(bytesRead, sizeof(bytesRead), "%0.02f", totalBytesRead);
    ...
    if (driveInfo->totalBytesWritten > 0)
    {
        ...
        snprintf(bytesWritten, sizeof(bytesWritten), "%0.02f", totalBytesWritten);
    ...
    {
        char utilization[68];
        snprintf(utilization, sizeof(utilization), "%0.04f", driveInfo->deviceReportedUtilizationRate);
    ...
    if (driveInfo->cacheSize > 0)
    {
        ...
        snprintf(cacheSizeVal, sizeof(cacheSizeVal), "%0.02f", cacheSize);
    ...
    if (driveInfo->rotationRate == 0x0001)
    {
        if (driveInfo->percentEnduranceUsed >= 0)
        {
            ...
            snprintf(enduranceUsed, sizeof(enduranceUsed), "%0.05f", driveInfo->percentEnduranceUsed);
    ...
    if (driveInfo->rotationRate == 0x0001 && driveInfo->totalWritesToFlash > 0)
    {
        if (driveInfo->totalLBAsWritten > 0)
        {
            ...
            snprintf(writeAmplification, sizeof(writeAmplification), "%0.02f",
    ...
    if (driveInfo->lowCurrentSpinupValid)
    {
        if (driveInfo->lowCurrentSpinupViaSCT) // to handle differences in reporting between 2.5" products and others
        {
            ...
            default: {
                ...
                snprintf(unknownState, sizeof(unknownState), "Unknown/Invalid state: %" PRIX16,
    ...
    {
        char vendorID[64];
        snprintf(vendorID, sizeof(vendorID), "%04" PRIX32 "h\n", driveInfo->adapterInformation.vendorID);
    ...
    {
        char productID[64];
        snprintf(productID, sizeof(productID), "%04" PRIX32 "h\n", driveInfo->adapterInformation.productID);
    ...
    {
        char revision[64];
        snprintf(revision, sizeof(revision), "%04" PRIX32 "h\n", driveInfo->adapterInformation.revision);
    ...
    {
        char specifierID[64];
        snprintf(specifierID, sizeof(specifierID), "%04" PRIX32 "h\n", driveInfo->adapterInformation.specifierID);
...
eReturnValues create_JSON_Node_For_NVMe_Device_Information(json_object* rootObject, ptrDriveInformationNVMe driveInfo)
{
    ...
    {
        char ieeeOUI[10];
        snprintf(ieeeOUI, sizeof(ieeeOUI), "%06" PRIX32, driveInfo->controllerData.ieeeOUI);
    ...
    char pciVendorID[8];
    snprintf(pciVendorID, sizeof(pciVendorID), "%04" PRIX16, driveInfo->controllerData.pciVendorID);
    json_object_object_add(nvmeInfoObject, "PCI Vendor ID", json_object_new_string(pciVendorID));

    char pciSubVendorID[8];
    snprintf(pciSubVendorID, sizeof(pciSubVendorID), "%04" PRIX16, driveInfo->controllerData.pciSubsystemVendorID);
    ...
    {
        char controllerID[8];
        snprintf(controllerID, sizeof(controllerID), "%04" PRIX16, driveInfo->controllerData.controllerID);
    ...
    {
        char nvmeVersion[15];
        snprintf(nvmeVersion, sizeof(nvmeVersion), "%" PRIu16 ".%" PRIu8 ".%" PRIu8,
    ...
    if (memcmp(zero128Bit, driveInfo->controllerData.fguid, 16) != 0)
    {
        ...
        {
            char temp[4];
            snprintf(temp, size(temp), "%02" PRIX8, driveInfo->controllerData.fguid[i]);
    ...
    if (driveInfo->controllerData.totalNVMCapacityD > 0)
    {
        ...
        char driveCapacity[64];
        snprintf(driveCapacity, sizeof(driveCapacity), "%0.02f/%0.02f", mTotalCapacity, totalCapacity);
        ...
        if (driveInfo->controllerData.unallocatedNVMCapacityD > 0)
        {
            ...
            snprintf(unNVMCapacity, sizeof(unNVMCapacity), "%0.02f/%0.02f", mUnCapacity, unCapacity);
    ...
    if (driveInfo->smartData.valid)
    {
        ...
        char poh[64];
        snprintf(poh, sizeof(poh), "%0.00f", driveInfo->smartData.powerOnHoursD);
        ...
        if (driveInfo->controllerData.longDSTTimeMinutes > UINT64_C(0))
        {
            ...
            // Append each time component to the string if it's greater than 0
            create_Time_String(&timeString, &years, &days, &hours, &minutes, &seconds);
            json_object_object_add(nvmeInfoObject, "Long Drive Self Test Time", json_object_new_string(timeString));
        }
        else
        {
...
#    define MINUTES_IN_1_YEAR 525600.0
    ...
    if (driveInfo->namespaceData.valid)
    {
        ...
        snprintf(namespaceSize, sizeof(namespaceSize), "%0.02f/%0.02f", nvmMSize, nvmMSize);
        ...
        snprintf(namespaceCapacity, sizeof(namespaceCapacity), "%0.02f/%0.02f", nvmMCap, nvmMCap);
        ...
        char utilization[64];
        snprintf(utilization, sizeof(utilization), "%0.02f/%0.02f", nvmMUtilization, nvmMUtilization);
        ...
        if (driveInfo->namespaceData.nvmCapacityD > 0)
        {
            ...
            capacity_Unit_Convert(&capacity, &capUnit);
            char cap[64];
            snprintf(cap, sizeof(cap), "%0.02f/%0.02f", mCapacity, capacity);
...
eReturnValues create_JSON_Output_For_Drive_Information(tDevice* device, bool showChildInformation, char** jsonFormat)
{
    ...
    ret = get_Drive_Information(device, ataDriveInfo, scsiDriveInfo, usbDriveInfo, nvmeDriveInfo, showChildInformation);
        ...
        else
        {
                ...
                ataDriveInfo && scsiDriveInfo && device->drive_info.drive_type == ATA_DRIVE)
            {
                ...
                if (usbDriveInfo != M_NULLPTR)
                {
                    ...
                    ret = create_JSON_For_Device_Information(usbDriveInfo);
                }
                else
                {
                    ret = MEMORY_FAILURE;
            ...
            else // ata or scsi
            {
                ...
                {
                    ret = create_JSON_For_Device_Information(ataDriveInfo);
                }
                else if (device->drive_info.drive_type == NVME_DRIVE && nvmeDriveInfo)
                {
                    ret = create_JSON_For_Device_Information(nvmeDriveInfo);
                }
                else if (scsiDriveInfo != M_NULLPTR)
                {
                    ret = create_JSON_For_Device_Information(scsiDriveInfo);
                }
                else
                {
