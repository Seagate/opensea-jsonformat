                                                                  ...
                                                                  char** jsonFormat)
{
    ...
    if (deviceStatictics->sataStatistics.vendorSpecificStatisticsSupported)
    {
        ...
        for (uint8_t vendorSpecificIter = 0, statisticsFound = 0;
             ...
             statisticsFound < deviceStatictics->sataStatistics.vendorSpecificStatisticsPopulated;
             ++vendorSpecificIter)
        {
             ...
             ++portIter)
        {
            if (deviceStatictics->sasStatistics.sasProtStats.sasStatsPerPort[portIter].sasProtStatsValid)
            {
                for (uint8_t phyIter = 0;
                     ...
                     phyIter < deviceStatictics->sasStatistics.sasProtStats.sasStatsPerPort[portIter].phyCount;
                     ++phyIter)
                {
                    if (deviceStatictics->sasStatistics.sasProtStats.sasStatsPerPort[portIter]
                            .perPhy[phyIter]
                            .sasPhyStatsValid)
                    {
                        atleastOneStatisticsAvailable = true;
...
eReturnValues create_JSON_Output_For_Device_Statistics(tDevice*                   device,
                                                       ptrDeviceStatistics        deviceStatictics,
                                                       ptrSeagateDeviceStatistics seagateDeviceStatistics,
                                                       bool                       seagateDeviceStatisticsAvailable,
                                                       char**                     jsonFormat)
